...
configure_homeserver_yaml() {
	...
	awk -v TURNURIES="turn_uris: [\"turn:${SERVER_NAME}:3478?transport=udp\", \"turn:${SERVER_NAME}:3478?transport=tcp\"]" \
		...
		-v CACHEFACTOR="synctl_cache_factor: 10" \
		-v WEBCLIENT="web_client: False" \
		-v BLOCKNONADMIN="block_non_admin_invites: True" \
		-v EVENTCACHE="event_cache_size: \"200K\"" \
		-v URLPREVIEW="url_preview_enabled: True" \
		-v IPRANGEBLACKLIST="url_preview_ip_range_blacklist:" \
		-v LOCALIPBLACKLIST=" - '127.0.0.0/8'" \
		-v ENABLEREGISTRATION="enable_registration: True" \
		-v AUTOJOINROOM="auto_join_rooms:" \
		-v AUTOJOINONE=" - \"#community:riot.tenx.tech\" \n - \"#announcements:riot.tenx.tech\"" \
	    '{
		...
		sub(/# group_creation_prefix: "unofficial\/"/, CACHEFACTOR);
		sub(/web_client: True/, WEBCLIENT);
		sub(/# block_non_admin_invites: True/, BLOCKNONADMIN);
		sub(/event_cache_size: "10K"/, EVENTCACHE);
		sub(/url_preview_enabled: False/,URLPREVIEW);
		sub(/# url_preview_ip_range_blacklist:/, IPRANGEBLACKLIST);
		sub(/# - \047127.0.0.0\/8\047/, LOCALIPBLACKLIST);
		sub(/enable_registration: False/, ENABLEREGISTRATION);
		sub(/#auto_join_rooms:/, AUTOJOINROOM);
		sub(/#    - "#example:example.com"/, AUTOJOINONE);
		print;
	    }' "${filepath}" > "${ymltemp}"
